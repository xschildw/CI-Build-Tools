...
# signature=$(echo -n "$sig_data" | iconv -t UTF8 | openssl dgst -sha1 -hmac "$decoded_key" | base64)
signature=$(printf $sig_data | openssl dgst -binary -sha1 -hmac "$decoded_key" | od -An -vtx1 | sed 's/[ \n]//g' | sed 'N;s/\n//')
